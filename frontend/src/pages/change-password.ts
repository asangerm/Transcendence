...
export async function renderChangePassword() {
  ...
  app.innerHTML = `
    <div class="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
      <div class="w-full max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-md p-8">
        ...
        <form id="change-password-form" class="flex flex-col gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Ancien mot de passe</label>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await UserService.changePassword(user.id, oldPassword, newPassword);
